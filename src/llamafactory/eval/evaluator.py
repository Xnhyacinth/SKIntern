...
class Evaluator:
    def __init__(self, args: Optional[Dict[str, Any]] = None) -> None:
        self.model_args, self.data_args, self.eval_args, self.finetuning_args, self.generating_args = get_eval_args(args)
        self.tokenizer = load_tokenizer(self.model_args)["tokenizer"]
        self.tokenizer.padding_side = "right"  # avoid overflow issue in batched inference for llama2
        self.template = get_template_and_fix_tokenizer(self.tokenizer, self.data_args.template)
        if not self.eval_args.vllm or self.eval_args.thop:
    ...
    def eval_gen(self):
        ...
        if self.eval_args.thop:
            print('\nCalculate Flops...\n')
            flops = 0
            for ins in tqdm(inputs, desc='cal flops'):
        ...
        if self.eval_args.gen_chat:
            ...
            if mt == 'mt-cas':
                ...
                if self.eval_args.thop:
                    ...
                    torch.cuda.reset_max_memory_cached()
                    self.model = load_model(self.tokenizer, self.model_args, self.finetuning_args)
                    for prompt in tqdm(user_prompts, desc='mt-cas  stage 2'):
