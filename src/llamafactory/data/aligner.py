...
def convert_mysft(
    ...
    elif dataset_attr.method == "inter":
        system_prompt = "{prefix} {task} Your response should conclude with the format \"Therefore, the answer is\".\n\nYou can utilize the general learning summary and supplementary knowledge proposed by the expert teacher to address this question.\n{summary}"
    else:
    ...
    fewshot = ""
    if data_args.inter_stage is not None:
        indices = list(range(len(examples[dataset_attr.prompt])))
        selected_indices = random.sample(indices, 6 - int(data_args.inter_stage))
        for idx in selected_indices:
            fewshot += "\n\nQuestion: " + examples[dataset_attr.prompt][idx] + "\nAnswer: " + examples[dataset_attr.response][idx]
    ...
    for i in range(len(examples[dataset_attr.prompt])):
        ...
        if dataset_attr.method == "std" or 'mt-' in dataset_attr.method or dataset_attr.method == 'ft':
            ...
            content.append(system_prompt.format(prefix=examples[dataset_attr.prefix][i], task=examples[dataset_attr.task_desc][i], summary=examples[dataset_attr.summary][i]) + fewshot + '\n')
